...
timeout --signal=TERM --kill-after=3 0 \
 ...
 ENGINE_LAB_INPUT=1 ENGINE_LAB_NATIVE_CAPTURE=1 ENGINE_LAB_PLAYBACK_CHANNELS=4 ALSASHIM_CARD="$card_index" ALSASHIM_MIDI_CARD="$card_index" ALSASHIM_BUFFER_SCALE=8 ENGINE_LAB_STEMS_MOUNT=/secure-media/stems-child-process.sqsh LD_PRELOAD=/root/pi-stems-directory.so:/root/pi-stems-affinity.so:/root/dtshim.so:/root/lab-input.so:/root/alsashim.so:/root/native-frame.so:/root/arm-ftz.so:/root/pcm-channels.so QT_QPA_PLATFORM=eglfs QT_QPA_EGLFS_INTEGRATION=eglfs_kms QT_QPA_EGLFS_FB=/dev/null QT_QPA_EGLFS_WIDTH=1280 QT_QPA_EGLFS_HEIGHT=800 QT_QPA_EGLFS_NO_LIBINPUT=1 QT_QPA_EVDEV_TOUCHSCREEN_PARAMETERS=/dev/input/pi-touch QT_QPA_EGLFS_HIDECURSOR=1 __EGL_VENDOR_LIBRARY_FILENAMES=/opt/pi-graphics/50_mesa.json LIBGL_DRIVERS_PATH=/opt/pi-graphics GBM_BACKENDS_PATH=/opt/pi-graphics \

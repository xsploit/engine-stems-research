...
int main(int argc,char**argv){
 if(argc!=7){fprintf(stderr,"usage: lib threads optimization output model runs\n");return 2;}
 ...
 double t=now();ck(a->CreateSession(env,argv[5],so,&s));double init=now()-t;
 ...
 const char*in[]={"input"};const char*out[]={"output"};const OrtValue*inputs[]={x};int runs=atoi(argv[6]);if(runs<1||runs>20)return 2;double times[20];OrtValue*y=NULL;
 for(int i=0;i<runs;i++){t=now();ck(a->Run(s,NULL,in,inputs,1,out,1,&y));times[i]=now()-t;if(i<runs-1){a->ReleaseValue(y);y=NULL;}}
 ...
 printf("{\"runtime\":\"%s\",\"threads\":%d,\"optimization\":%d,\"init_seconds\":%.9f,\"first_run_seconds\":%.9f,\"warm_seconds\":[",base()->GetVersionString(),threads,opt,init,times[0]);for(int i=1;i<runs;i++)printf("%s%.9f",i==1?"":",",times[i]);printf("],\"finite\":true}\n");

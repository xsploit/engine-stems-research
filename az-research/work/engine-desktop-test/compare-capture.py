...
b=next((root/'private-capture').glob('stdout-*.private')).read_bytes();i=0;chunks=[];nch=0;headers=0;done=False
while i<len(b):
...
base=root.parents[1]/'outputs/engine-pc-prototype/pc-cpu4';candidate=np.concatenate([readwav(base/f'stem-{s}.wav') for s in range(4)],axis=1)

...
## Round 4 (closure) changes
- U3 closed: the latency-drop copy loop destination is scratch block +0xaf8 (0xecac/0xed8c), not host output.
- U4 closed: Hopping buffers cleared in Hopping::prepare 0x173b4; history 0x14dcc; aligned_alloc 0x215f4 does not zero. StemsPro::reset has no caller in main.
- U5 closed: mono mask O[s*2]*I[0] (0x15de0), EM(s,0,f) (0x15e9c), EM Est index s*M+c (0x128f0), residual out index 4 (0x161d0).
- New N1: synthesis Nyquist re[2048] is never written (copyFFTResult buf[1]=re[n-1], 0x253cc; setMagnitudeAndPhase count 2048). Content is allocator dependent.
- U1: pffft wrapper conventions consistent with upstream "F(0)+iF(N/2)" packing; no scaling constant in pffft_transform_internal. Still H-strong.
- U2: flushBuffer emits min(block, FIFO fill); finishProcessing adjusts the write index at 0xf4d8/0xf730; exact formula is H-strong, corroborated by historical frame counts.
- See PIPELINE-SPEC.md for the consolidated, non-contradictory spec.

## Round 5 corrections (supersede round 4 N1/U2 text)
- CORRECTION N1: round 4 said the synthesis Nyquist bin was "never written; allocator garbage". That was WRONG. Absence of a found clear was not proof. prepare 0x13f88-0x14010 memsets both halves (2049 bins) of every channel of complex block +0x690 (frames/capacity 2049 set at 0x13dd4). setMagnitudeAndPhase writes 2048 bins; transformInverse reads const. So re[2048]=0 and synthesis Nyquist = 0.
  Scope of evidence: fields +0x6a0..+0x6f8 are referenced only in the ctor/prepare/releaseResources/reset/dtor and computeOutputTensor (global grep of 10-proc-full.txt). No other alias was found.
- U2 decoded: finish writes n+L frames (single block, L <= B-n) or B + (n+L-B) (two blocks), with a late L drop if the stream is shorter than B. Host frames = T. See PIPELINE-SPEC.md section 6 and 19-trim-arithmetic.txt.
- U1 still H-strong: zreorder loop structure matches upstream; semantics/scale not derivable without full rfftf1_ps algebra or source.

## Round 6 (identity pass)
- PFFFT: reclassified as a source-corroborated assumption per PFFFT-SOURCE-NOTE.md (no binary-revision equivalence).
- Identity BLOCKED: the Engine parent is stripped. Label xrefs are map/QML registrations without channel indices; the stems protocol/writer region is 0xc28400-0xc2b400. Details are in PIPELINE-SPEC.md section 9.
